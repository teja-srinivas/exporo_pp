...
class ProcessBillCreation implements ShouldQueue
{
    ...
    public function handle($type = 'pdf')
    {
        ...
        return $this->getRequest($urlBoxUrl);
    }
    ...
    protected function getRequest(string $url)
    {
        ...
        Storage::disk('s3')->put('statements/' . $this->bill['id'] . '-' . $this->bill->user['last_name'] . '-' . $this->bill->user['first_name'] . '-' . $this->bill['created_at'] . '.pdf'
